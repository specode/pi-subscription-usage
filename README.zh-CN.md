...
- **Grok**：每周和/或每月额度；只使用 Pi 的 `xai` / `xai-auth` OAuth 凭据，并先验证账户身份。统一账单账户在 `?format=credits` 缺少窗口百分比时，会再读取默认的月度账单接口。窗口与其他提供商一样使用 `5h / 1w / 1m` 状态格式。
- **Kimi Coding**：5 小时和每周窗口。
...
每次调用都会跳过缓存并重新查询当前提供商。所有额度窗口都使用统一格式，并以 `MM/DD HH:mm` 显示重置时间。
...
不同额度域的窗口不会交错。需要刷新时再次运行 `/usage` 即可；命令不会显示刷新、切换提供商或查询所有提供商的菜单。
